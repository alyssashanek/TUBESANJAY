...
void drawButtonLabel(const char* text, int x, int y, int width) {
    ...
    // posisi tulisan dibikin ke tengah tombol
...
void drawStartScreen() {
    ...
    // nyalain depth dan lighting
    glPushAttrib(GL_ALL_ATTRIB_BITS);
    glEnable(GL_DEPTH_TEST);
    glEnable(GL_LIGHTING);
    glEnable(GL_LIGHT0);

    // warna langit dan cahaya buat mode siang/malam
    if (isDaySelected) {
    ...
    // kalau siang pencahayaannya dibikin lebih terang
    if (isDaySelected) {
        treeAmbient[0] =
        treeAmbient[1] =
        treeAmbient[2] = 0.5f;
    ...
    // gambar dekorasi sekitar track
    drawTrees();
    drawClouds();
    drawBillboard();
    drawTrophy();

    // tampilkan matahari atau bulan
    if (isDaySelected)
        drawSun();
    else
        drawMoon();

    // render mobil di menu utama
    glPushMatrix();
    glTranslatef(-35.0f, 2.0f, -5.0f);
    glRotatef(0.0f, 0, 1, 0);
    glScalef(1.15f, 1.15f, 1.15f);

    // simpan mode lampu sebelumnya
    int oldHeadlight = headlightMode;

    // matiin headlight pas render menu
    headlightMode = 0;
    ...
    // gambar overlay hitam transparan
    glEnable(GL_BLEND);
    ...
    int titleWidth = glutBitmapLength(
    ...
    // tombol DAY
    glColor3f(
        isDaySelected ? 0.0f : 0.3f, 
        isDaySelected ? 1.0f : 0.6f, 
        0.0f
    );
    ...
    drawButtonLabel(
        "Day", 
        startX, dayY + buttonHeight / 2, 
        buttonWidth
    );

    // tombol NIGHT
    glColor3f(
        isNightSelected ? 0.2f : 0.1f, 
        isNightSelected ? 0.2f : 0.1f, 
        isNightSelected ? 1.0f : 0.6f
    );
    ...
    glPopAttrib();
     ...
     // tampilkan hasil render ke layar
    glutSwapBuffers();
}

// buat nampilin teks biasa di layar
void drawText(const char* string, int x, int y) {
    glDisable(GL_LIGHTING);
    glColor3f(!day, !day, !day); 
    glRasterPos2i(x, y); 

    // render karakter satu-satu
    while (*string) {
...
void renderCenteredText(const char* string) {
    ...
    // hitung posisi tengah layar kira-kira
    int x = glutGet(GLUT_WINDOW_WIDTH) / 2 - strlen(string) * 4.5;
...
// indikator kecepatan mobil
void drawMPHDial(float mph) {
    // tinggi bar speedometer
    float gaugeHeight = 20.0f; 

    // posisi awal speed bar
    int baseX = 10;
    int baseY = glutGet(GLUT_WINDOW_HEIGHT) - 30;
    glColor3f(1.0f, 1.0f, 1.0f);

    // panjang bar sesuai kecepatan
    glBegin(GL_QUADS);
    ...
    // tampilkan status drive/reverse
    drawText(
        (velocity >= 0) ? "DRIVE" : "REVERSE", 
        10, 965
    );
}
